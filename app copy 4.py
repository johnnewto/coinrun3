...
with col1:
    st.subheader("GDP")
    gdp_chart = st.line_chart(x_label="Years", y_label="GDP")

with col2:
    st.subheader("Debt (% of GDP)")
    debt_chart = st.line_chart(x_label="Years", y_label="Debt % GDP")

# Add a new row for Money Supply and Bank Accounts
with col1:
    st.subheader("Money Supply and Bank Accounts")
    money_chart = st.line_chart(x_label="Years", y_label="Money")

with col2:
    st.subheader("Interest Payments (% of GDP)")
    int_chart = st.line_chart(x_label="Years", y_label="Interest % GDP")

# Real-time simulation
for _ in range(total_steps):  # Use the slider value for total steps
